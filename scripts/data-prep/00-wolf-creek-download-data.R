...
solar1 <- read_csv('data/wolf-creek/met/rosy/WCFtwr1_SWrad_20132018.csv') |>
  mutate(
    datetime = as.POSIXct(date_time_UTC, tz = tz),
  ...
  select(
    ...
    albedo_Avg_14m
  ) # UP vs Down are mislabelled in the header
...
solar2 <- read_csv('data/wolf-creek/met/rosy/WCFmet_SWrad_20172022.csv') |>
  ...
  select(
    datetime,
    sw_in_18m = SW_DN_18m_Avg,
    sw_out_18m = SW_UP_18m_Avg,
    albedo_Avg_18m
  )

solar <- rbind(
  solar1 |> pivot_longer(!datetime),
  solar2 |> pivot_longer(!datetime)
) |>
  rbind(
    solar_mod |>
...
wha_in <- readRDS(
  'data/wolf-creek/met/eccc/weathercan_whitehorse_airport.rds'
) |>
