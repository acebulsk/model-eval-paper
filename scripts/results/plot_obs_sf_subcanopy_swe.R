...
ggplot(all_sites_hru_sf, aes(datetime, cml_sf)) +
  geom_point(
    data = all_sites_obs,
    aes(x = datetime, y = obs_swe, colour = "Forest Snow Survey"),
    inherit.aes = FALSE
  ) +
  geom_line(aes(colour = "Cumulative Snowfall (Clearing)")) +
  facet_wrap(~station, nrow = 4, scales = 'free') +
  labs(y = 'Snow Water Equivilent (mm)', x = element_blank(), colour = element_blank()) +
